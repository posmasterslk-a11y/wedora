  ...
  <div class="relative" ref="dropdownRef">
    ...
    >
      ...
      <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar">
        <li v-if="!notifications || notifications.length === 0" class="p-4 text-center text-sm text-gray-500">No new RSVPs yet.</li>
        <li v-else v-for="notification in notifications" :key="notification.id">
          <router-link
            :to="'/invites/' + notification.invite_id + '/rsvps'"
...
const { data: notifications } = useAsyncData('recent-rsvps', async () => {
  ...
  // Get recent RSVPs for these invitations
  ...
  if (rsvps && rsvps.length > 0) notifying.value = true
  
  return rsvps?.map(r => {
    const invite = invites.find(i => i.id === r.invitation_id)
    return {
      id: r.id,
      userName: r.guest_name,
      userImage: `https://ui-avatars.com/api/?name=${encodeURIComponent(r.guest_name)}&background=random`,
      action: r.is_attending ? `RSVP'd Yes (${r.guest_count} guests)` : "Regretfully Declined",
      project: invite?.couple_names || 'Wedding',
      type: 'RSVP',
      time: new Date(r.created_at).toLocaleDateString(),
      status: r.is_attending ? 'online' : 'offline',
      invite_id: r.invitation_id
    }
  }) || []
}, { watch: [user] })
